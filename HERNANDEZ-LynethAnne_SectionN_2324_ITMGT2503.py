...
for key, value in jsondata.items():
    jsondata[key] = str(value).upper()
    for text in remove:
        if text in jsondata[key]:
            jsondata[key] = jsondata[key].replace(text, '')
    for punctuation in unnecessary_punctuation:
        if punctuation in jsondata[key]:
            jsondata[key] = jsondata[key].replace(punctuation, ' ')
...
for line in jsondata.values():
    ...
    for word in word_list:
        if len(word) >= 3:
            if word in wordcount_dictionary.keys():
...
for key, value in wordcount_dictionary.items():
    final_wordlist_of_tuples.append((key, value))
...
max_F = min(18000 // 1000, 12000 // 500, 1500 // 50)
max_R = min(18000 // 600, 12000 // 750, 1500 // 45)
max_C = min(18000 // 750, 12000 // 300, 1500 // 75)
optimal_Z = 0
optimal_F = 0
optimal_R = 0
optimal_C = 0
...
def Collatz(start_number):
    ...
    collatz_dict = {"collatz_sequence": [], "collatz_length":0, "max_collatz":0,"sequence_sum":0}
    
    collatz_dict["collatz_sequence"].append(start_number)
    ...
    collatz_dict["collatz_length"] = len(collatz_dict["collatz_sequence"])
    collatz_dict["max_collatz"] = max(collatz_dict["collatz_sequence"])
    collatz_dict["sequence_sum"] = sum(collatz_dict["collatz_sequence"])
...
#Just copied Collatz function for referencing purposes for Collatz_winner function
def Collatz(start_number):
    collatz_dict = {"collatz_sequence": [], "collatz_length":0, "max_collatz":0,"sequence_sum":0}
...
def Collatz_winner(number_list):
    ...
    length_list = []
    max_list = []
    winning_candidates_len = []
    winning_candidates_max = []
    
    for num in number_list:
        col_length = Collatz(num)['collatz_length']
        length_list.append((num, col_length))
        col_max = Collatz(num)['max_collatz']
        max_list.append((num, col_max))
    
    length_list.sort(key=lambda x: x[1], reverse=True)
    max_list.sort(key=lambda x: x[1], reverse=True)
    
    winning_candidates_len.append(length_list[0][0])
    winning_candidates_max.append(length_list[0][0])
    
    for i in range(1, len(length_list)):
        if length_list[0][0] == length_list[i][0]:
            winning_candidates_len.append(length_list[i][0])
            
    for i in range(1, len(max_list)):
        if max_list[0][0] == max_list[i][0]:
            winning_candidates_max.append(length_list[i][0])
    
    winner_set = set(winning_candidates_len).intersection(set(winning_candidates_max))
